...
While we can find different techniques for each category, there are some behaviors that are shared and it is wise to always follow them.
...
* _**Not everything is perfect**, or as [Sturgeon’s Law](https://en.wikipedia.org/wiki/Sturgeon%27s_law) mentions “ninety percent of everything is crap”. This means that not everything deserves our attention and our valuable time. Sometimes it is better if we let some behaviors slip, acknowledging that they are bad but not worthy of our time and effort._

## **Providing Feedback**

A lot of times we are in a place where we want to give feedback about a behavior. Other times we want to mention something that was annoying and caused us negative feelings. In cases like these, providing timely feedback is usually the right way to avoid building up complicated situations.

### **Feedback**

When our goal is to provide feedback about a behavior it’s always good to follow the [SBI](https://www.ccl.org/articles/leading-effectively-articles/sbi-feedback-model-a-quick-win-to-improve-talent-conversations-development/) (**Situation-Behavior-Impact**) framework. SBI framework advises us to 1) describe the exact situation, 2) followed by the observed behavior that caused the issue and 3) lastly the actual impact it had on us. Let’s see this in a real example. What follows is an example of a conversation between a couple of colleagues.
...
In the end, remember that we should try to keep criticism to the minimum every time. Giving criticism about too many points, we jeopardize that the message we want to transmit will not be received with success.
...
> “Hey Bob, you keep on repeating the same behavior when we are in a meeting and I end up being frustrated! Please stop doing that!”
